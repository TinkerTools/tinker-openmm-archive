...
!$OMP& i12,i13,i14,i15,v2scale,v3scale,v4scale,v5scale,uselambda
!$OMP& ,use_group,off2,radmin,epsilon,radmin4,epsilon4,ghal,dhal,
!$OMP& vlambda,scalpha,scexp,cut2,c0,c1,c2,c3,c4,c5,use_relative,
!$OMP& molcule,name,verbose,debug,header,iout,mut1,mut2)
...
c
         do kk = 1, nvlst(ii)
            ...
            if (use_group)  call groups (proceed,fgrp,i,k,0,0,0,0)
            if (proceed)  proceed = (usei .or. use(k) .or. use(kv))
            if (proceed) proceed = (.not.(mut1(k) .and. mut2(i)))
            if (proceed) proceed = (.not.(mut1(i) .and. mut2(k)))
c
c     compute the energy contribution for this interaction
c
            if (proceed) then
...
c     get the interaction energy, via soft core if necessary
c
        if((mut1(i).eqv. mut1(k)) .and. (mut2(k) .eqv.
     &           mut2(i))) then
                        uselambda=1.0
                 else if(mut1(i) .or. mut1(k)) then              
                        uselambda = vlambda
                 else
                        uselambda= 1.0-vlambda
                 end if
                     rho = rik / rv
                     eps = eps * uselambda**scexp
                     scal = scalpha * (1.0d0-uselambda)**2
                     t1 = (1.0d0+dhal)**7 / (scal+(rho+dhal)**7)
                     t2 = (1.0d0+ghal) / (scal+rho**7+ghal)
                     e = eps * t1 * (t2-2.0d0)                 
c

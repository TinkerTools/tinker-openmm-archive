...
//
static double getCFluxDirection(int atomA, int atomB) {
   double cfluxDir;
   int atomicA, atomicB, n12a, n12b, nha, nhb; 
   int i, atomIndex;
   //  0: no charge flux
   // +1: charge flux from atom1--->atom2
   // -1: charge flux from atom2--->atom1
   cfluxDir = 0.0f;
   atomicA = atomid__.atomic[atomA];
   atomicB = atomid__.atomic[atomB]; 
   n12a = *(couple__.n12 + atomA);
   n12b = *(couple__.n12 + atomB);
   nha = 0;
   nhb = 0;
   for (i = 0; i < n12a; i++) {
      atomIndex = *(couple__.i12 + sizes__.maxval*atomA + i) - 1;
      if (atomid__.atomic[atomIndex] == 1) {
        nha += 1; 
      }
   }

   for (i = 0; i < n12b; i++) {
      atomIndex = *(couple__.i12 + sizes__.maxval*atomB + i) - 1;
      if (atomid__.atomic[atomIndex] == 1) {
        nhb += 1; 
      }
   }
   //printf("atomA atomicA n12a nha %i %i %i %i \n", atomA, atomicA, n12a, nha);
   //printf("atomB atomicB n12b nhb %i %i %i %i \n", atomB, atomicB, n12b, nhb);
   cfluxDir = (atomicA != atomicB ? (atomicA > atomicB ? -1.0f : 1.0f) : 
              ((n12a  != n12b ? (n12a > n12b ? -1.0f : 1.0f) : 
               (nha != nhb ? (nha > nhb ? -1.0f : 1.0f) : 0.0f))));
...
static void setupAmoebaMultipoleForce (OpenMM_System* system, FILE* log) {
   ...
   // pass cflux bond parameters to openmm
   int atom1, atom2;
   double cfDir;
   double jParameterConversion = (double) 1.0/OpenMM_NmPerAngstrom;
   bondPtr = bndstr__.ibnd;
   for (ii = 0; ii < bndstr__.nbond; ii++) {
      atom1 = (*bondPtr)-1;
      atom2 = *(bondPtr+1)-1;

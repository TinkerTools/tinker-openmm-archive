...
void ReferenceCalcAmoebaVdwForceKernel::initialize(const System& system, const AmoebaVdwForce& force) {
    ...
    for (int ii = 0; ii < numParticles; ii++) {
        ...
        int indexIV, vtype,groupNum;
        double sigma, epsilon, reduction, lambda;
        std::vector<int> exclusions;

        force.getParticleParameters(ii, indexIV, vtype, sigma, epsilon, reduction, lambda,groupNum);
...
void ReferenceCalcAmoebaVdwForceKernel::copyParametersToContext(ContextImpl& context, const AmoebaVdwForce& force) {
    ...
    for (int ii = 0; ii < numParticles; ii++) {
        ...
        int indexIV, vtype,ligGroup;
        double sigma, epsilon, reduction, lambda;
        std::vector<int> exclusions;

        force.getParticleParameters(ii, indexIV, vtype, sigma, epsilon, reduction, lambda,ligGroup);
        force.getParticleExclusions(ii, exclusions);
        for (unsigned int jj = 0; jj < exclusions.size(); jj++) {
            allExclusions[ii].insert(exclusions[jj]);
        }

...
public:
    ...
    bool areParticlesIdentical(int particle1, int particle2) {
        int vtype1, vtype2,ligGroup1, ligGroup2;
        int parentIndex;
        double sigma, epsilon, reductionFactor, lambda1, lambda2;
        force.getParticleParameters(particle1, parentIndex, vtype1, sigma, epsilon, reductionFactor, lambda1,ligGroup1);
        force.getParticleParameters(particle2, parentIndex, vtype2, sigma, epsilon, reductionFactor, lambda2,ligGroup2);
...
void CudaCalcAmoebaVdwForceKernel::initialize(const System& system, const AmoebaVdwForce& force) {
    ...
    numGroups = CudaArray::create<float>(cu, cu.getPaddedNumAtoms(), "numGroups");
    ...
    vector<float> numGroupsVec(cu.getPaddedNumAtoms(), 0);
	...
	int numGroup;
        ...
        force.getParticleParameters(i, ivIndex, vType, sigma, epsilon, reductionFactor, vlambda,numGroup);
        vdwTypesVec[i] = vType;
	numGroupsVec[i] = static_cast<float>(numGroup);
    ...
    nonbonded->addParameter(CudaNonbondedUtilities::ParameterInfo("numGroups", "float", 1, sizeof(int), numGroups->getDevicePointer()));
	...
	nonbonded->addInteraction(useCutoff, useCutoff, true, force.getCutoff(), exclusions,                                 
        cu.replaceStrings(CudaAmoebaKernelSources::amoebaVdwForce2relative, replacements), 0);     
    } 
...
double CudaCalcAmoebaVdwForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    ...
    void* prepareArgs[] = { &cu.getForce().getDevicePointer(),
        &cu.getPosq().getDevicePointer(),
        &tempPosq->getDevicePointer(),

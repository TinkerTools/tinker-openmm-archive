...
void ReferenceCalcAmoebaMultipoleForceKernel::initialize(const System& system, const AmoebaMultipoleForce& force) {
    ...
    penalphas.resize(numMultipoles);
    pencores.resize(numMultipoles);
    tholes.resize(numMultipoles);
    dirdamps.resize(numMultipoles);
    ...
    for (int ii = 0; ii < numMultipoles; ii++) {
        ...
        double charge, penalphaD, pencoreD, tholeD, dirdampD, dampingFactorD, polarityD;
        ...
        force.getMultipoleParameters(ii, charge, dipolesD, quadrupolesD, axisType, multipoleAtomZ, multipoleAtomX, multipoleAtomY,
                                     penalphaD, pencoreD, tholeD, dirdampD, dampingFactorD, polarityD);
        ...
        penalphas[ii]                      = static_cast<RealOpenMM>(penalphaD);
        pencores[ii]                       = pencoreD; 
        tholes[ii]                         = static_cast<RealOpenMM>(tholeD);
        dirdamps[ii]                       = static_cast<RealOpenMM>(dirdampD);
...
double ReferenceCalcAmoebaMultipoleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    ...
    vector<RealVec>& posData   = extractPositions(context);
    vector<RealVec>& forceData = extractForces(context);
    RealOpenMM energy          = amoebaReferenceMultipoleForce->calculateForceAndEnergy(posData, charges, dipoles, quadrupoles, penalphas, pencores, tholes, dirdamps,
...
void ReferenceCalcAmoebaMultipoleForceKernel::getInducedDipoles(ContextImpl& context, vector<Vec3>& outputDipoles) {
    ...
    amoebaReferenceMultipoleForce->calculateInducedDipoles(posData, charges, dipoles, quadrupoles, penalphas, pencores, tholes, dirdamps,
...
void ReferenceCalcAmoebaMultipoleForceKernel::getLabFramePermanentDipoles(ContextImpl& context, vector<Vec3>& outputDipoles) {
    ...
    amoebaReferenceMultipoleForce->calculateLabFramePermanentDipoles(posData, charges, dipoles, quadrupoles, penalphas, pencores, tholes, dirdamps,
...
void ReferenceCalcAmoebaMultipoleForceKernel::getTotalDipoles(ContextImpl& context, vector<Vec3>& outputDipoles) {
    ...
    amoebaReferenceMultipoleForce->calculateTotalDipoles(posData, charges, dipoles, quadrupoles, penalphas, pencores, tholes, dirdamps,
...
void ReferenceCalcAmoebaMultipoleForceKernel::getElectrostaticPotential(ContextImpl& context, const std::vector< Vec3 >& inputGrid,
    ...
    amoebaReferenceMultipoleForce->calculateElectrostaticPotential(posData, charges, dipoles, quadrupoles, penalphas, pencores, tholes, dirdamps,
                                                                   dampingFactors, polarity, axisTypes,
                                                                   multipoleAtomZs, multipoleAtomXs, multipoleAtomYs,
...
void ReferenceCalcAmoebaMultipoleForceKernel::getSystemMultipoleMoments(ContextImpl& context, std::vector< double >& outputMultipoleMoments) {
    ...
    AmoebaReferenceMultipoleForce* amoebaReferenceMultipoleForce = setupAmoebaReferenceMultipoleForce(context);
    vector<RealVec>& posData                                     = extractPositions(context);
    amoebaReferenceMultipoleForce->calculateAmoebaSystemMultipoleMoments(masses, posData, charges, dipoles, quadrupoles, penalphas, pencores, tholes, dirdamps,
                                                                         dampingFactors, polarity, axisTypes,
                                                                         multipoleAtomZs, multipoleAtomXs, multipoleAtomYs,
...
void ReferenceCalcAmoebaMultipoleForceKernel::copyParametersToContext(ContextImpl& context, const AmoebaMultipoleForce& force) {
    ...
    for (int i = 0; i < numMultipoles; ++i) {
        ...
        std::vector<double> dipolesD;
        std::vector<double> quadrupolesD;
        force.getMultipoleParameters(i, charge, dipolesD, quadrupolesD, axisType, multipoleAtomZ, multipoleAtomX, multipoleAtomY, penalphaD, pencoreD, tholeD, dirdampD, dampingFactorD, polarityD);
        ...
        penalphas[i] = (RealOpenMM) penalphaD;
        pencores[i] = pencoreD;
        tholes[i] = (RealOpenMM) tholeD;
        dirdamps[i] = (RealOpenMM) dirdampD;
...
void ReferenceCalcAmoebaGeneralizedKirkwoodForceKernel::initialize(const System& system, const AmoebaGeneralizedKirkwoodForce& force) {
    ...
    for (int ii = 0; ii < numParticles; ii++) {
        ...
        amoebaMultipoleForce->getMultipoleParameters(ii, charge2, dipole, quadrupole, axisType, atomZ, atomX, atomY, penalpha, pencore, thole, dirdamp, damping, polarity);
...
void ReferenceCalcAmoebaVdwForceKernel::initialize(const System& system, const AmoebaVdwForce& force) {
    functionalForm = force.getFunctionalForm();
...
double ReferenceCalcAmoebaVdwForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    ...
    if (functionalForm == "BUFFERED-14-7") {
        vdwForce.setFunctionalForm(AmoebaReferenceVdwForce::BUFFERED_14_7);
...
ReferenceCalcAmoebaCTForceKernel::ReferenceCalcAmoebaCTForceKernel(std::string name, const Platform& platform, const System& system)
...
ReferenceCalcAmoebaCTForceKernel::~ReferenceCalcAmoebaCTForceKernel() {
    if (neighborList) {
        delete neighborList;
    }
}

void ReferenceCalcAmoebaCTForceKernel::initialize(const System& system, const AmoebaCTForce& force) {

    // per-particle parameters

    numParticles = system.getNumParticles();
    numCTprTypes = force.getNumCTprTypes();

    //indexIVs.resize(numParticles);
    allExclusions.resize(numParticles);
    CTprTypes.resize(numParticles);
    combinedApres.resize(numCTprTypes * numCTprTypes);
    combinedBexps.resize(numCTprTypes * numCTprTypes);
    for (int i = 0; i < numCTprTypes; ++i) {
        for (int j = 0; j < numCTprTypes; ++j) {
            ...
            combinedApres[k] = combinedApre;
            combinedBexps[k] = combinedBexp;
        }
    }
    ...
    useCutoff = (force.getNonbondedMethod() != AmoebaCTForce::NoCutoff);
    usePBC = (force.getNonbondedMethod() == AmoebaCTForce::CutoffPeriodic);
    cutoff = force.getCutoff();
    neighborList = useCutoff ? new NeighborList() : NULL;
}

double ReferenceCalcAmoebaCTForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    vector<RealVec>& posData = extractPositions(context);
    vector<RealVec>& forceData = extractForces(context);
    AmoebaReferenceCTForce CTForce = AmoebaReferenceCTForce();
    RealOpenMM energy;
    if (useCutoff) {
        CTForce.setCutoff(cutoff);
        computeNeighborListVoxelHash(*neighborList, numParticles, posData, allExclusions, extractBoxVectors(context), usePBC, cutoff, 0.0);
        if (usePBC) {
            CTForce.setNonbondedMethod(AmoebaReferenceCTForce::CutoffPeriodic);
            RealVec* boxVectors = extractBoxVectors(context);
            double minAllowedSize = 1.999999 * cutoff;
            if (boxVectors[0][0] < minAllowedSize || boxVectors[1][1] < minAllowedSize || boxVectors[2][2] < minAllowedSize) {
                throw OpenMMException("The periodic box size has decreased to less than twice the cutoff.");
            }
            CTForce.setPeriodicBox(boxVectors);
            energy = CTForce.calculateForceAndEnergy(numParticles, numCTprTypes, CTprTypes,
                posData, combinedApres, combinedBexps, 
                *neighborList, forceData);
        }
        else {
            CTForce.setNonbondedMethod(AmoebaReferenceCTForce::CutoffNonPeriodic);
        }
    }
    else {
        CTForce.setNonbondedMethod(AmoebaReferenceCTForce::NoCutoff);
        energy = CTForce.calculateForceAndEnergy(numParticles, numCTprTypes, CTprTypes,
            posData, combinedApres, combinedBexps, allExclusions,
            forceData);
    }
    return static_cast<double>(energy);
}

void ReferenceCalcAmoebaCTForceKernel::copyParametersToContext(ContextImpl& context, const AmoebaCTForce& force) {
    if (numParticles != force.getNumParticles())
        throw OpenMMException("updateParametersInContext: The number of particles has changed");

...
private:
    ...
    std::vector<RealOpenMM> penalphas;
    std::vector<RealOpenMM> pencores;
    std::vector<RealOpenMM> tholes;
    std::vector<RealOpenMM> dirdamps;
 ...
 * This kernel is invoked to calculate the vdw forces acting on the system and the energy of the system.
 */
class ReferenceCalcAmoebaCTForceKernel : public CalcAmoebaCTForceKernel {
public:
    ReferenceCalcAmoebaCTForceKernel(std::string name, const Platform& platform, const System& system);
    ~ReferenceCalcAmoebaCTForceKernel();
    /**
     * Initialize the kernel.
     *
     * @param system     the System this kernel will be applied to
     * @param force      the AmoebaCTForce this kernel will be used for
     */
...
private:
    ...
    double cutoff;
    std::vector< std::set<int> > allExclusions;
    std::vector<int> CTprTypes;
    std::vector<RealOpenMM> combinedApres;
    std::vector<RealOpenMM> combinedBexps;
    const System& system;
    NeighborList* neighborList;
};

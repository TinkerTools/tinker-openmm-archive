...
private:
    ...
    CudaArray* predefCharges;
    CudaArray* cfCharges;
    CudaArray* cfPotentials;
    CudaArray* bondCFluxParams;
    CudaArray* atomIndicesBond;
    CudaArray* angleCFluxAngleParams;
    CudaArray* angleCFluxBondParams;
    CudaArray* atomIndicesAngle;
    ...
    CudaArray* dampedPotentialValues;
    ...
    CudaSort* sort;
    cufftHandle fft;
    CUfunction computeBondCFluxesKernel;
    CUfunction computeAngleCFluxesKernel;
    CUfunction computeCFluxBondForceKernel;
    CUfunction computeCFluxAngleForceKernel;
    std::vector<void*> computeBondCFluxesArgs; 
    std::vector<void*> computeAngleCFluxesArgs; 
    std::vector<void*> computeCFluxBondForceArgs; 
    std::vector<void*> computeCFluxAngleForceArgs; 
...
public:
     ...
     */
    void finishComputation(CudaArray& torque, CudaArray& labFrameDipoles, CudaArray& labFrameQuadrupoles, CudaArray& inducedDipole, CudaArray& inducedDipolePolar, CudaArray& penAlpha, CudaArray& penCore, CudaArray& dampingAndThole, CudaArray& dirDamping, CudaArray& covalentFlags, CudaArray& polarizationGroupFlags);
...
/**
 * This kernel is invoked to calculate the CT forces acting on the system and the energy of the system.
 */
class CudaCalcAmoebaCTForceKernel : public CalcAmoebaCTForceKernel {
public:
    CudaCalcAmoebaCTForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system);
    ~CudaCalcAmoebaCTForceKernel();
    /**
     * Initialize the kernel.
     *
     * @param system     the System this kernel will be applied to
     * @param force      the AmoebaMultipoleForce this kernel will be used for
     */
    void initialize(const System& system, const AmoebaCTForce& force);
    /**
     * Execute the kernel to calculate the forces and/or energy.
     *
     * @param context        the context in which to execute this kernel
     ...
     *
     * @param context    the context to copy parameters to
     * @param force      the AmoebaCTForce to copy the parameters from
     */
    void copyParametersToContext(ContextImpl& context, const AmoebaCTForce& force);
private:
    class ForceInfo;
    CudaContext& cu;
    const System& system;

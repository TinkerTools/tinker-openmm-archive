...
public:
    ...
    bool areParticlesIdentical(int particle1, int particle2) {
        double charge1, charge2, penalpha1, penalpha2, pencore1, pencore2, thole1, thole2, dirdamp1, dirdamp2, damping1, damping2, polarity1, polarity2;
        ...
        force.getMultipoleParameters(particle1, charge1, dipole1, quadrupole1, axis1, multipole11, multipole21, multipole31, penalpha1, pencore1, thole1, dirdamp1, damping1, polarity1);
        force.getMultipoleParameters(particle2, charge2, dipole2, quadrupole2, axis2, multipole12, multipole22, multipole32, penalpha2, pencore2, thole2, dirdamp2, damping2, polarity2);
        if (charge1 != charge2 || penalpha1 != penalpha2 || pencore1 != pencore2 || thole1 != thole2 || dirdamp1 != dirdamp2 || damping1 != damping2 || polarity1 != polarity2 || axis1 != axis2) {
            return false;
        }
...
CudaCalcAmoebaMultipoleForceKernel::CudaCalcAmoebaMultipoleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
        ...
        fracDipoles(NULL), fracQuadrupoles(NULL), field(NULL), fieldPolar(NULL), inducedField(NULL), inducedFieldPolar(NULL), torque(NULL), penAlpha(NULL), penCore(NULL), dampingAndThole(NULL), dirDamping(NULL), inducedDipole(NULL),
        ...
        pmePhid(NULL), pmePhip(NULL), pmePhidp(NULL), pmeCphi(NULL), lastPositions(NULL), sort(NULL), gkKernel(NULL), predefCharges(NULL), cfCharges(NULL), bondCFluxParams(NULL), atomIndicesBond(NULL), 
        angleCFluxAngleParams(NULL), angleCFluxBondParams(NULL), atomIndicesAngle (NULL), cfPotentials(NULL) {
}

CudaCalcAmoebaMultipoleForceKernel::~CudaCalcAmoebaMultipoleForceKernel() {
    cu.setAsCurrent();
    if (cfPotentials != NULL)
        delete cfPotentials;
    if (predefCharges != NULL)
        delete predefCharges;
    if (cfCharges != NULL)
        delete cfCharges;
    if (bondCFluxParams != NULL)
        delete bondCFluxParams;
    if (atomIndicesBond != NULL)
        delete atomIndicesBond;
    if (angleCFluxAngleParams != NULL)
        delete angleCFluxAngleParams;
    if (angleCFluxBondParams != NULL)
        delete angleCFluxBondParams;
    if (atomIndicesAngle != NULL)
        delete atomIndicesAngle;
...
void CudaCalcAmoebaMultipoleForceKernel::initialize(const System& system, const AmoebaMultipoleForce& force) {
    ...
    vector<float> penAlphaVec;
    vector<float> penCoreVec;
    vector<float> dirDampingVec;
    ...
    vector<double> predefChargesVecd;
    vector<float> predefChargesVecf;

    predefCharges = new CudaArray(cu, numMultipoles, cu.getUseDoublePrecision() ? sizeof(double) : sizeof(float), "predefCharges"); 
    
    for (int i = 0; i < numMultipoles; i++) {
        double charge, penalpha, pencore, thole, dirdamp, damping, polarity;
        int axisType, atomX, atomY, atomZ;
        vector<double> dipole, quadrupole;
        force.getMultipoleParameters(i, charge, dipole, quadrupole, axisType, atomZ, atomX, atomY, penalpha, pencore, thole, dirdamp, damping, polarity);
        if (cu.getUseDoublePrecision()) {
            posqd[i] = make_double4(0, 0, 0, charge);
            predefChargesVecd.push_back(charge);
        
        } else { 
            posqf[i] = make_float4(0, 0, 0, (float) charge);
            predefChargesVecf.push_back((float) charge);
        }
 
        dampingAndTholeVec.push_back(make_float2((float) damping, (float) thole));
        penAlphaVec.push_back((float) penalpha);
        penCoreVec.push_back((float) pencore);
        dirDampingVec.push_back((float) dirdamp);
    ...
    if (cu.getUseDoublePrecision()) 
        predefCharges->upload(predefChargesVecd);
    else 
        predefCharges->upload(predefChargesVecf);
    ...
    for (int i = numMultipoles; i < paddedNumAtoms; i++) {
        ...
        penAlphaVec.push_back(0);
        penCoreVec.push_back(0);
        dirDampingVec.push_back(0);
    ...
    penAlpha = CudaArray::create<float>(cu, paddedNumAtoms, "penAlpha");
    penCore = CudaArray::create<float>(cu, paddedNumAtoms, "penCore");
    dirDamping = CudaArray::create<float>(cu, paddedNumAtoms, "dirDamping");
    ...
    penAlpha->upload(penAlphaVec);
    penCore->upload(penCoreVec);
    dirDamping->upload(dirDampingVec);
    ...
    startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
    numAngles = endIndex-startIndex;
    if (numAngles == 0)
        return;

    vector<vector<int> > atomsA(numAngles, vector<int>(3));
    vector<double3> angleCFluxAngleParamsVectord(numAngles);
    vector<double4> angleCFluxBondParamsVectord(numAngles);
    vector<float3> angleCFluxAngleParamsVectorf(numAngles);
    vector<float4> angleCFluxBondParamsVectorf(numAngles);
    vector<int3> atomIndicesAngleVector(numAngles);
    for (int i = 0; i < numAngles; i++) {
        double theta0, jtheta1, jtheta2, bond1, jbp1, bond2, jbp2; 
        force.getCFluxAngleParameters(startIndex+i, atomsA[i][0], atomsA[i][1], atomsA[i][2], 
                                      theta0, jtheta1, jtheta2, bond1, jbp1, bond2, jbp2);
        if (cu.getUseDoublePrecision()) 
        {
          angleCFluxAngleParamsVectord[i] = make_double3(theta0, jtheta1, jtheta2);
          angleCFluxBondParamsVectord[i] = make_double4(bond1, jbp1, bond2, jbp2); 
        }
        else 
        {
          angleCFluxAngleParamsVectorf[i] = make_float3(theta0, jtheta1, jtheta2);
          angleCFluxBondParamsVectorf[i] = make_float4(bond1, jbp1, bond2, jbp2); 
        }
        atomIndicesAngleVector[i] = make_int3((int) atomsA[i][0], (int) atomsA[i][1], (int) atomsA[i][2]);
    }
    // CudaArrays
    ...
    atomIndicesAngle = new CudaArray(cu, numAngles, 3*sizeof(int), "atomIndicesAngle");

    if (cu.getUseDoublePrecision()) {
      bondCFluxParams->upload(bondCFluxParamsVectord);
      angleCFluxAngleParams->upload(angleCFluxAngleParamsVectord);
      angleCFluxBondParams->upload(angleCFluxBondParamsVectord);
    }
    else {
      bondCFluxParams->upload(bondCFluxParamsVectorf);
      angleCFluxAngleParams->upload(angleCFluxAngleParamsVectorf);
      angleCFluxBondParams->upload(angleCFluxBondParamsVectorf);
    }
    atomIndicesBond->upload(atomIndicesBondVector);
    atomIndicesAngle->upload(atomIndicesAngleVector);

    map<string, string> definesCFlux;
    definesCFlux["NUM_BONDS"] = cu.intToString(numBonds);
    definesCFlux["NUM_ANGLES"] = cu.intToString(numAngles);
    definesCFlux["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    definesCFlux["APPLY_PERIODIC"] = (force.usesPeriodicBoundaryConditions() ? "1" : "0");
    definesCFlux["PADDED_NUM_ATOMS"] = cu.intToString(cu.getPaddedNumAtoms());
    CUmodule moduleCFlux = cu.createModule(CudaKernelSources::vectorOps+CudaAmoebaKernelSources::amoebaCFluxAndForce, definesCFlux);
    computeBondCFluxesKernel = cu.getKernel(moduleCFlux, "computeBondCFluxes");
    computeAngleCFluxesKernel = cu.getKernel(moduleCFlux, "computeAngleCFluxes");
    computeCFluxBondForceKernel = cu.getKernel(moduleCFlux, "computeCFluxBondForce");
    computeCFluxAngleForceKernel = cu.getKernel(moduleCFlux, "computeCFluxAngleForce");
    cu.addForce(new ForceInfo(force));
}
...
double CudaCalcAmoebaMultipoleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    ...
    // Compute the charge flux due to bond stretching 
    int numAtoms = cu.getNumAtoms();
    computeBondCFluxesArgs.push_back(&cu.getPosq().getDevicePointer());
    computeBondCFluxesArgs.push_back(cu.getPeriodicBoxSizePointer());
    computeBondCFluxesArgs.push_back(cu.getInvPeriodicBoxSizePointer());
    computeBondCFluxesArgs.push_back(cu.getPeriodicBoxVecXPointer());
    computeBondCFluxesArgs.push_back(cu.getPeriodicBoxVecYPointer());
    computeBondCFluxesArgs.push_back(cu.getPeriodicBoxVecZPointer());
    computeBondCFluxesArgs.push_back(&cfCharges->getDevicePointer());
    computeBondCFluxesArgs.push_back(&bondCFluxParams->getDevicePointer());
    computeBondCFluxesArgs.push_back(&atomIndicesBond->getDevicePointer());
    cu.executeKernel(computeBondCFluxesKernel, &computeBondCFluxesArgs[0], numBonds);

    // Compute the charge flux due to angle bending/(bond)stretching 
    computeAngleCFluxesArgs.push_back(&cu.getPosq().getDevicePointer());
    computeAngleCFluxesArgs.push_back(cu.getPeriodicBoxSizePointer());
    computeAngleCFluxesArgs.push_back(cu.getInvPeriodicBoxSizePointer());
    computeAngleCFluxesArgs.push_back(cu.getPeriodicBoxVecXPointer());
    computeAngleCFluxesArgs.push_back(cu.getPeriodicBoxVecYPointer());
    computeAngleCFluxesArgs.push_back(cu.getPeriodicBoxVecZPointer());
    computeAngleCFluxesArgs.push_back(&angleCFluxAngleParams->getDevicePointer());
    computeAngleCFluxesArgs.push_back(&angleCFluxBondParams->getDevicePointer());
    computeAngleCFluxesArgs.push_back(&cfCharges->getDevicePointer());
    computeAngleCFluxesArgs.push_back(&atomIndicesAngle->getDevicePointer());
    cu.executeKernel(computeAngleCFluxesKernel, &computeAngleCFluxesArgs[0], numAngles);
    ...
    // modify atomic charge on each atom
    ...
    void* computeMomentsArgs[] = {&cu.getPosq().getDevicePointer(), &multipoleParticles->getDevicePointer(),
        ...
        &predefCharges->getDevicePointer(), &cfCharges->getDevicePointer()};
    cu.executeKernel(computeMomentsKernel, computeMomentsArgs, cu.getNumAtoms());
    cu.clearBuffer(*cfCharges);
    ...
    if (pmeGrid == NULL) {
        ...
        if (gkKernel == NULL) {
            void* computeFixedFieldArgs[] = {&field->getDevicePointer(), &fieldPolar->getDevicePointer(), &cu.getPosq().getDevicePointer(),
                ...
                &labFrameDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &penAlpha->getDevicePointer(), &penCore->getDevicePointer(), &dampingAndThole->getDevicePointer(), &dirDamping->getDevicePointer()};
        ...
        void* electrostaticsArgs[] = {&cu.getForce().getDevicePointer(), &torque->getDevicePointer(), &cu.getEnergyBuffer().getDevicePointer(),
            ...
            &labFrameDipoles->getDevicePointer(), &sphericalDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(),&sphericalQuadrupoles->getDevicePointer(),
            &inducedDipole->getDevicePointer(), &inducedDipolePolar->getDevicePointer(), &penAlpha->getDevicePointer(), &penCore-> getDevicePointer(), &dampingAndThole->getDevicePointer(), &dirDamping->getDevicePointer()};
        ...
        if (gkKernel != NULL)
            gkKernel->finishComputation(*torque, *labFrameDipoles, *labFrameQuadrupoles, *inducedDipole, *inducedDipolePolar, *penAlpha, *penCore, *dampingAndThole, *dirDamping, *covalentFlags, *polarizationGroupFlags);
    }
    else {
        ...
        void* pmeFixedForceArgs[] = {&cu.getPosq().getDevicePointer(), &cu.getForce().getDevicePointer(), &torque->getDevicePointer(),
            ...
            recipBoxVectorPointer[0], recipBoxVectorPointer[1], recipBoxVectorPointer[2], &cfPotentials->getDevicePointer()};
        ...
        void* computeFixedFieldArgs[] = {&field->getDevicePointer(), &fieldPolar->getDevicePointer(), &cu.getPosq().getDevicePointer(),
            ...
            &labFrameDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &dampingAndThole->getDevicePointer(), &dirDamping->getDevicePointer()};
        cu.executeKernel(computeFixedFieldKernel, computeFixedFieldArgs, numForceThreadBlocks*fixedFieldThreads, fixedFieldThreads);
        void* recordInducedDipolesArgs[] = {&field->getDevicePointer(), &fieldPolar->getDevicePointer(),
            &inducedDipole->getDevicePointer(), &inducedDipolePolar->getDevicePointer(), &polarizability->getDevicePointer()};
        cu.executeKernel(recordInducedDipolesKernel, recordInducedDipolesArgs, cu.getNumAtoms());
        ...
        void* electrostaticsArgs[] = {&cu.getForce().getDevicePointer(), &torque->getDevicePointer(), &cu.getEnergyBuffer().getDevicePointer(),
            ...
            &maxTiles, &nb.getBlockCenters().getDevicePointer(), &nb.getInteractingAtoms().getDevicePointer(),
            &labFrameDipoles->getDevicePointer(), &sphericalDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &sphericalQuadrupoles->getDevicePointer(),
            &inducedDipole->getDevicePointer(), &inducedDipolePolar->getDevicePointer(), &penAlpha->getDevicePointer(),&penCore->getDevicePointer(), &dampingAndThole->getDevicePointer(), &dirDamping->getDevicePointer(), &cfPotentials->getDevicePointer()};
        cu.executeKernel(electrostaticsKernel, electrostaticsArgs, numForceThreadBlocks*electrostaticsThreads, electrostaticsThreads);
        ...
        void* pmeInducedForceArgs[] = {&cu.getPosq().getDevicePointer(), &cu.getForce().getDevicePointer(), &torque->getDevicePointer(),
            ...
            &pmePhip->getDevicePointer(), &pmePhidp->getDevicePointer(), &pmeCphi->getDevicePointer(), recipBoxVectorPointer[0], recipBoxVectorPointer[1], recipBoxVectorPointer[2], &cfPotentials->getDevicePointer()};
        cu.executeKernel(pmeInducedForceKernel, pmeInducedForceArgs, cu.getNumAtoms());

        // compute forces due to cflux bond
        
        computeCFluxBondForceArgs.push_back(&cu.getForce().getDevicePointer());
        computeCFluxBondForceArgs.push_back(&cu.getPosq().getDevicePointer());
        computeCFluxBondForceArgs.push_back(cu.getPeriodicBoxSizePointer());
        computeCFluxBondForceArgs.push_back(cu.getInvPeriodicBoxSizePointer());
        computeCFluxBondForceArgs.push_back(cu.getPeriodicBoxVecXPointer());
        computeCFluxBondForceArgs.push_back(cu.getPeriodicBoxVecYPointer());
        computeCFluxBondForceArgs.push_back(cu.getPeriodicBoxVecZPointer());
        computeCFluxBondForceArgs.push_back(&cfPotentials->getDevicePointer());
        computeCFluxBondForceArgs.push_back(&bondCFluxParams->getDevicePointer());
        computeCFluxBondForceArgs.push_back(&atomIndicesBond->getDevicePointer());
        cu.executeKernel(computeCFluxBondForceKernel, &computeCFluxBondForceArgs[0], numBonds);

        // Compute forces due to cflux angle 
        computeCFluxAngleForceArgs.push_back(&cu.getForce().getDevicePointer());
        computeCFluxAngleForceArgs.push_back(&cu.getPosq().getDevicePointer());
        computeCFluxAngleForceArgs.push_back(cu.getPeriodicBoxSizePointer());
        computeCFluxAngleForceArgs.push_back(cu.getInvPeriodicBoxSizePointer());
        computeCFluxAngleForceArgs.push_back(cu.getPeriodicBoxVecXPointer());
        computeCFluxAngleForceArgs.push_back(cu.getPeriodicBoxVecYPointer());
        computeCFluxAngleForceArgs.push_back(cu.getPeriodicBoxVecZPointer());
        computeCFluxAngleForceArgs.push_back(&angleCFluxAngleParams->getDevicePointer());
        computeCFluxAngleForceArgs.push_back(&angleCFluxBondParams->getDevicePointer());
        computeCFluxAngleForceArgs.push_back(&cfPotentials->getDevicePointer());
        computeCFluxAngleForceArgs.push_back(&atomIndicesAngle->getDevicePointer());
        cu.executeKernel(computeCFluxAngleForceKernel, &computeCFluxAngleForceArgs[0], numAngles);
        cu.clearBuffer(*cfPotentials);
    } 
...
void CudaCalcAmoebaGeneralizedKirkwoodForceKernel::initialize(const System& system, const AmoebaGeneralizedKirkwoodForce& force) {
    ...
    for (int i = 0; i < force.getNumParticles(); i++) {
        ...
        double charge2, penalpha, pencore, thole, dirdamp, damping, polarity;
        int axisType, atomX, atomY, atomZ;
        vector<double> dipole, quadrupole;
        multipoles->getMultipoleParameters(i, charge2, dipole, quadrupole, axisType, atomZ, atomX, atomY, penalpha, pencore, thole, dirdamp, damping, polarity);
        if (charge != charge2)
...
void CudaCalcAmoebaGeneralizedKirkwoodForceKernel::finishComputation(CudaArray& torque, CudaArray& labFrameDipoles, CudaArray& labFrameQuadrupoles,
            CudaArray& inducedDipole, CudaArray& inducedDipolePolar, CudaArray& penAlpha, CudaArray& penCore, CudaArray& dampingAndThole, CudaArray& dirDamping, CudaArray& covalentFlags, CudaArray& polarizationGroupFlags) {
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int startTileIndex = nb.getStartTileIndex();
    int numTileIndices = nb.getNumTiles();
    int numForceThreadBlocks = nb.getNumForceThreadBlocks();
    ...
    void* ediffArgs[] = {&cu.getForce().getDevicePointer(), &torque.getDevicePointer(), &cu.getEnergyBuffer().getDevicePointer(),
        ...
        &dampingAndThole.getDevicePointer(), &dirDamping.getDevicePointer()};
...
class CudaCalcAmoebaCTForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const AmoebaCTForce& force)
        : force(force) {}
...
CudaCalcAmoebaCTForceKernel::CudaCalcAmoebaCTForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system)
    : CalcAmoebaCTForceKernel(name, platform)
    , cu(cu)
    , system(system)
    , hasInitializedNonbonded(false)
    , CTTypes(NULL)
...
void CudaCalcAmoebaCTForceKernel::initialize(const System& system, const AmoebaCTForce& force) {
    cu.setAsCurrent();
    ...
    map<string, string> replacements;
    replacements["PADDED_NUM_ATOMS"] = cu.intToString(cu.getPaddedNumAtoms());
    replacements["NUM_CTPR_TYPES"] = cu.intToString(xsize);
    double cutoff = force.getCutoffDistance();
    double taperCutoff = cutoff * 0.9;
    replacements["CUTOFF_DISTANCE"] = cu.doubleToString(force.getCutoffDistance());
    replacements["TAPER_CUTOFF"] = cu.doubleToString(taperCutoff);
...
double CudaCalcAmoebaCTForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
   if (!hasInitializedNonbonded) 
   {
       hasInitializedNonbonded = true;
       nonbonded->initialize(system);
   }
   nonbonded->prepareInteractions(1); //groups
   nonbonded->computeInteractions(1, true, true);//groups, includeForce, includeEnergy

...
typedef struct {
    ...
    real q;
    real potential;
    float penalpha, pencore, thole, damp, dirdamp;
...
inline __device__ void loadAtomData(AtomData& data, int atom, const real4* __restrict__ posq, const real* __restrict__ sphericalDipole,
        ...
        const real* __restrict__ penAlpha, const real* __restrict__ penCore) {
...
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, bool hasExclusions, float dScale, float pScale, float mScale, float forceFactor,
    ...
    real u = fabs(dmp) > 1.0e-5f ? SQRT(r/dmp) : 1e10f;
    real au3 = a*u*u*u;
    ...
    eUIndCoef = rInvVec[2]*(pScale*mbfunc_c + bVec[2]);
    eUInpCoef = rInvVec[2]*(dScale*mbfunc_c + bVec[2]);

    dPermCoef = -rInvVec[3]*(mScale + bVec[2] + alphaRVec[3]*X);
    dUIndCoef = -2*rInvVec[3]*(pScale*dmbfunc_c + bVec[2] + alphaRVec[3]*X);
    dUInpCoef = -2*rInvVec[3]*(dScale*dmbfunc_c + bVec[2] + alphaRVec[3]*X);
    ...
    eUIndCoef = -twoThirds*rInvVec[3]*(3*(pScale*mbfunc_d0 + bVec[3]) + alphaRVec[3]*X);
    eUInpCoef = -twoThirds*rInvVec[3]*(3*(dScale*mbfunc_d0 + bVec[3]) + alphaRVec[3]*X);
    dPermCoef = rInvVec[4]*(3*(mScale + bVec[3]) + 2*alphaRVec[5]*X);
    dUIndCoef = rInvVec[4]*(6*(pScale*dmbfunc_d0 + bVec[3]) + 4*alphaRVec[5]*X);
    dUInpCoef = rInvVec[4]*(6*(dScale*dmbfunc_d0 + bVec[3]) + 4*alphaRVec[5]*X);
    ...
    eUIndCoef = rInvVec[3]*(pScale*mbfunc_d1 + bVec[3] - twoThirds*alphaRVec[3]*X);
    eUInpCoef = rInvVec[3]*(dScale*mbfunc_d1 + bVec[3] - twoThirds*alphaRVec[3]*X);

    dPermCoef = -1.5f*rInvVec[4]*(mScale + bVec[3]);
    dUIndCoef = -3*rInvVec[4]*(pScale*dmbfunc_d1 + bVec[3]);
    dUInpCoef = -3*rInvVec[4]*(dScale*dmbfunc_d1 + bVec[3]);
    ...
    eUIndCoef = rInvVec[4]*(3*(pScale*mbfunc_q0 + bVec[3]) + fourThirds*alphaRVec[5]*X);
    eUInpCoef = rInvVec[4]*(3*(dScale*mbfunc_q0 + bVec[3]) + fourThirds*alphaRVec[5]*X);
    dPermCoef = -fourThirds*rInvVec[5]*(4.5f*(mScale + bVec[3]) + (1 + alphaRVec[2])*alphaRVec[5]*X);
    dUIndCoef = -fourThirds*rInvVec[5]*(9*(pScale*dmbfunc_q0 + bVec[3]) + 2*(1 + alphaRVec[2])*alphaRVec[5]*X);
    dUInpCoef = -fourThirds*rInvVec[5]*(9*(dScale*dmbfunc_q0 + bVec[3]) + 2*(1 + alphaRVec[2])*alphaRVec[5]*X);
    ...
    eUIndCoef = -sqrtThree*rInvVec[4]*(pScale*mbfunc_q1 + bVec[3]);
    eUInpCoef = -sqrtThree*rInvVec[4]*(dScale*mbfunc_q1 + bVec[3]);
    const real fourSqrtOneThird = 4/sqrt((real) 3);
    dPermCoef = fourSqrtOneThird*rInvVec[5]*(1.5f*(mScale + bVec[3]) + 0.5f*alphaRVec[5]*X);
    dUIndCoef = fourSqrtOneThird*rInvVec[5]*(3*(pScale*dmbfunc_q1 + bVec[3]) + alphaRVec[5]*X);
    dUInpCoef = fourSqrtOneThird*rInvVec[5]*(3*(dScale*dmbfunc_q1 + bVec[3]) + alphaRVec[5]*X);
    ...
    //
    atom1.potential += Vij[0];
    if (forceFactor == 1) {
        atom2.force -= force;
        atom2.potential += Vji[0];
...
__device__ void computeOneInteractionCP(AtomData& atom1, AtomData& atom2, bool hasExclusions, float mScale, float forceFactor,
                                      mixed& energy, real4 periodicBoxSize, real4 invPeriodicBoxSize, real4 periodicBoxVecX, real4 periodicBoxVecY, real4 periodicBoxVecZ) {
  ...
  //charge penetration damping functions
    ...
    real nuci = (real) atom1.pencore;
    real nuck = (real) atom2.pencore;       
    ...
    real term1i = nuck*scalei_1 + qk*scaleik_1; 
    real term1k = nuci*scalek_1 + qi*scaleik_1; 
    real term2i = -drk*scaleik_3; 
    real term2k = +dri*scaleik_3; 
    real term3i = qrrk*scaleik_5;
    real term3k = qrri*scaleik_5;

    real poti = term1i*rr1 + term2i*rr3 + term3i*rr5;
    real potk = term1k*rr1 + term2k*rr3 + term3k*rr5;
    ...
    atom1.potential += poti; 
    if (forceFactor == 1) {
      atom2.potential += potk; 
      atom2.force -= forceCP;
...
__device__ void computeSelfEnergyAndTorque(AtomData& atom1, mixed& energy) {
    ...
    atom1.potential += 2.0f*prefac*atom1.q;
    //printf("self: %f \n", 2.0f*prefac*atom1.q*ENERGY_SCALE_FACTOR/4.184f); //correct
...
extern "C" __global__ void computeElectrostatics(
...
#endif
        const real* __restrict__ labFrameDipole, const real* __restrict__ sphericalDipole, const real* __restrict__ labFrameQuadrupole,
        const real* __restrict__ sphericalQuadrupole, const real* __restrict__ inducedDipole,
        const real* __restrict__ inducedDipolePolar, const real* __restrict__ penAlpha, const real* __restrict__ penCore, 
        const float2* __restrict__ dampingAndThole, const real* __restrict__ dirDamping, real* __restrict__ cfPotentials) {
    ...
    for (int pos = firstExclusionTile; pos < lastExclusionTile; pos++) {
        ...
        loadAtomData(data, atom1, posq, sphericalDipole, sphericalQuadrupole, inducedDipole, inducedDipolePolar, dampingAndThole, dirDamping, 
                     labFrameDipole, labFrameQuadrupole, penAlpha, penCore);
        ...
        if (x == y) {
            ...
            localData[threadIdx.x].potential = data.potential;
            ...
            localData[threadIdx.x].pencore = data.pencore;
            ...
            for (unsigned int j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 != atom2 && atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    //printf("same tile indices: atom1 atom2 %i %i \n", atom1, atom2); 
                    ...
                    computeOneInteractionCP(data, localData[tbx+j], true, m, 0.5f, energy, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
                }
            }
            if (atom1 < NUM_ATOMS)
            ...
            data.potential *= ENERGY_SCALE_FACTOR;

            atomicAdd(&cfPotentials[atom1], data.potential);
            ...
            loadAtomData(localData[threadIdx.x], j, posq, sphericalDipole, sphericalQuadrupole, inducedDipole, inducedDipolePolar, dampingAndThole, dirDamping,
                         labFrameDipole, labFrameQuadrupole, penAlpha, penCore);
            localData[threadIdx.x].potential = 0.0f;
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    //printf("diff tile indices: atom1 atom2 %i %i \n", atom1, atom2); 
                    ...
                    computeOneInteractionCP(data, localData[tbx+tj], true, m, 1, energy, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
            data.force *= -ENERGY_SCALE_FACTOR;
            ...
            atomicAdd(&cfPotentials[offset], (localData[threadIdx.x].potential));
    ...
    while (pos < end) {
        ...
        if (includeTile) {
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    //printf("no exclusions: atom1 atom2 %i %i \n", atom1, atom2); // 
                    ...
                    computeOneInteractionCP(data, localData[tbx+tj], false, 1, 1, energy, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
            data.potential *= ENERGY_SCALE_FACTOR;
